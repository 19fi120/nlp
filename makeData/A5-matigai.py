...
for i,path in enumerate(dirs3):
    new_filelist.append(path+"/A5-matigai.dat")
...
for i,file in enumerate(filelist):
    ...
    """予測した文字列の自立語のリストを作る"""
    juman = []
    with open(jumanlist[i],"r",encoding="cp932") as fileobj:
        for z,line in enumerate(fileobj):
            aline = line.rstrip()
            datalist = aline.split(" ")
            juman.append(datalist)
    
    """正解データのリストを作る"""
    answer = [] #[[0文節目の係り先ID,完全一致用の文字列,部分一致用の文字列]
    with open(answerlist[i],"r",encoding="utf_8") as fileobj:
        ...
        for a,line in enumerate(fileobj) :
            ...
            if line.find("*") == 0:
                ...
                str1 += datalist[0]
                if datalist[3] != "助詞" and datalist[3] != "助動詞" and datalist[3] != "接尾辞" and datalist[3] != "特殊":
                    str2 += datalist[0]
        tmp.append(str1)
        tmp.append(str2)
        answer.append(tmp)
        ...
        if int(line[0]) == 0:   #既入力の文節が追加される
            ...
            for c in range(count):
                if int(answer[c][0]) >= count and int(answer[c][0]) != -1:
                    mi_set.add(answer[c][0])
            mia += len(mi_set)
            exNum = 0   #未入力係り先文節が正解と対応付けされてないときに、それを区別するためのID
            
        if int(line[0]) == -1:
            if len(minyuuryoku_set) == 0: break
            ...
            if int(line[0]) in minyuuryoku_set: #文字列の評価、ifの条件などはこのプログラムの一番下
                if len(line[2]) > 0:
                    mo += 1
                    try:
                        isMatch = False     #文字列の自立語が一致しているか
                        for e,a in enumerate(juman[b]):
                            if answer[answerdict[int(line[0])]][2].count(a) > 0 and juman[b][0] != '':
                                bubun += 1
                                isMatch = True
                                break
                        ...
                        if not isMatch:
                            tigau += 1
                            data = answerdict[int(line[0])]
                            print(data)
                            new_fileobj.write(str(data)+"\n")
                    except:
                        tigau += 1
                        exNum += 1
                        data = "e" + str(exNum)
...
print(f"文字列を部分的に予測した数 {bubun}")
print(f"文字列を部分的に予測出来なかった数 {tigau}")
print(f"文字列を入力しなかった数　{NE}")
