...
"""仕分け"""
path = "../HumanIncreParse/HumanIncreParse/SubjA"
dirs1 = []  #年月日
...
for i,file in enumerate(filelist):
    ...
    """文節ごとの作業結果のリストを作る"""
    bunlist = []    #[["文節ID","予測した係り先文節ID","得点"],...,["未入力文節ID","対応する正解の文節ID","予測した文字列","得点"],...]
    with open(file,"r",encoding="utf_8") as fileobj:
        for y,line in enumerate(fileobj) :            
            ...
            aline = line.rstrip()
            datalist = aline.split(" ")
            bunlist.append(datalist)
    
    """評価"""
    for x,line in enumerate(bunlist) :
        if int(line[0]) < 0:    #未入力文節に入ったら終了
            break
        num += 1
